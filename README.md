...
### 0) 引用回复适配器
- **无需工具调用**：LLM 直接在输出中使用 `[REPLY:message_id]内容` 格式即可引用回复
- **不接管发送流程**：只负责将 `[REPLY:...]` 标签转换为 Reply 消息组件，不干预 AstrBot 原有的分段发送逻辑
- **可选提示词注入**：通过 `prompt` 配置项自定义引导提示词，为空则不注入
- **同一消息内换行**：使用 `\n`（字面两个字符）表示换行

> **配置简化**：现在只有 2 个配置项：`enable`（开关）和 `prompt`（提示词，为空则不注入）。
...
> ⚠️ **注意**：`reply_message` 工具已弃用并移除，请使用**引用回复适配器**代替（见上方功能说明）。
...
### 0) 引用回复适配器
启用 `reply_adapter.enable=true` 后，LLM 可使用引用回复格式输出：

```text
[REPLY:1234567]收到，我来回复这条消息
这是普通消息，不带引用
[REPLY:8888888]这是另一条引用回复\n这里是同一消息的第二行
```
...
```json
{
  "reply_adapter": {
    "enable": true,
    "prompt": "【引用回复格式】当你需要引用回复某条消息时，请在输出中使用：[REPLY:消息ID]你的回复内容。消息ID可从上下文中的 [MSG_ID:xxx] 获取。如果回复内容需要换行，使用 \\n 表示（字面两个字符）。不使用此格式则为普通消息。"
  }
}
```

> **注意**：`enable` 默认为 `false`。`prompt` 有默认值，如果你想自己在人设中配置引导，可以设置为空字符串。
